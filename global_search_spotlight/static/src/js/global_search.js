 ...
 * (16.0+, since the OWL2 web client rewrite) by plain declarative
 * `t-on-*` directives directly in the QWeb template - OWL2's template
 * compiler handles delegation/binding natively and more efficiently.
 * That's what this module uses below
...
export class SpotlightSystray extends Component {
    ...
    setup() {
        ...
        // needed on our end.
        //
        // Odoo 16.0 note: this module was originally written against
        // later versions and ports cleanly - `useHotkey`/`hotkey_hook`
        // and its options (bypassEditableProtection, global) are part of
        // the OWL2 hotkey service introduced with the 16.0 web client
        // rewrite, and 16.0's own built-in Command Palette already
        // depends on it for its Ctrl+K binding. That said, if the hotkey
        // doesn't fire for some reason on your build, the systray icon
        // click (`toggle()` below) always works regardless - it's a
        // completely independent trigger path.
