...
export class OwAccountingPremiumDashboard extends Component {
    ...
    companyDomain(field = "company_id") {
        const companyId = this.companyId || this.companyService.allowedCompanyIds?.[0];
    ...
    async loadCompany() {
        try {
            const companyId = this.companyService.allowedCompanyIds?.[0];
    ...
    async loadCashBalance() {
        try {
            const accounts = await this.orm.searchRead(
                "account.account",
                [["account_type", "=", "asset_cash"], ...this.companyDomain()],
                ["id"],
                { limit: 1000 }
            );
